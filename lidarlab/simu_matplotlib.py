...
QUEUE = mp.Queue() 
PROCESS = mp.Process(target=MPL_worker, args=(QUEUE,))
PROCESS.start() 
...
def convert_to_cartesian(angles, distances):
    ...
    for angle, distance in zip(angles, distances):
        ...
        # Hint, Pythagoras' theorem can be used to convert polar to cartesian coordinates
        cartesian_X = distance*np.cos(angle*VAL_PI/180) #REPLACE ME with the correct formula
        cartesian_Y = distance*np.sin(angle*VAL_PI/180) #REPLACE ME with the correct formula
